...
type Props = { scenario: Scenario; readOnly?: boolean; fontSize: number; onChange:(doc:string, meta:{paste:boolean;undo:boolean})=>void; onKey?:()=>void; onMode?:(mode:string)=>void }
export function VimEditor({ scenario, readOnly=false, fontSize, onChange, onKey, onMode }: Props) {
  const host = useRef<HTMLDivElement>(null); const readOnlyCompartment = useRef(new Compartment()); const view = useRef<EditorView>()
  const callback = useRef({onChange,onKey,onMode}); callback.current={onChange,onKey,onMode}
  useEffect(() => {
    ...
    try { Vim.defineEx?.('write','w',()=>{}); const status = document.querySelector('.cm-vim-panel'); if(status) callback.current.onMode?.(status.textContent||'NORMAL') } catch { /* optional Vim telemetry */ }
  ...
  return <div className="editor-shell" ref={host} onFocus={()=>onMode?.('NORMAL')} />
}
