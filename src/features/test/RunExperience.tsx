...
export function RunExperience({scenario,mode,onNext,onRandom}:{scenario:Scenario;mode:'test'|'practice';onNext?:()=>void;onRandom?:()=>void}){
  const {prefs,runs,addRun}=useApp(); const [attempt,setAttempt]=useState(0); const session=useRef(new GameSession(scenario)); const [status,setStatus]=useState(session.current.status); const [elapsed,setElapsed]=useState(0); const [showHint,setShowHint]=useState(false); const [keys,setKeys]=useState(0); const [writeMessage,setWriteMessage]=useState('')
  const previousBest=bestFor(runs,scenario.id,mode)
  useEffect(()=>{ session.current=new GameSession(scenario);setStatus('ready');setElapsed(0);setKeys(0);setShowHint(false);setWriteMessage('') },[scenario,attempt])
  ...
  const write=()=>{if(status==='completed'){onNext?.();return}setWriteMessage('Target not matched — keep editing.')}
  ...
  return <section className="run-experience">
    ...
    <div className="diff-workspace">
      <section className="diff-pane diff-source" aria-label="Editable document">
        ...
        <VimEditor key={`${scenario.id}-${attempt}`} scenario={scenario} readOnly={status==='completed'} fontSize={prefs.fontSize} onKey={()=>setKeys(k=>k+1)} onChange={change} onWrite={write} onReset={reset} onNew={onRandom}/>
    ...
    {status==='completed'?<div className="result-card"><div className="result-icon"><Check/></div><div><span className="eyebrow">{isPb?'New personal best':'Challenge complete'}</span><h2>{formatTime(elapsed)} <small>{keys} keystrokes</small></h2>{mode==='practice'&&scenario.reference?.suggestedSolution&&<p>Suggested: <code>{scenario.reference.suggestedSolution}</code></p>}</div><div className="result-actions"><Button variant="outline" onClick={reset}><RotateCcw size={16}/>Retry <kbd>:e</kbd></Button>{onRandom&&<Button variant="outline" onClick={onRandom}><Shuffle size={16}/>Random <kbd>:n</kbd></Button>}{onNext&&<Button onClick={onNext}>Next drill <kbd>:w</kbd><ArrowRight size={16}/></Button>}</div></div>:<div className="under-editor"><div aria-live="polite" className={writeMessage?'write-message':''}><Zap size={16}/> {writeMessage||'Match the target exactly to finish.'}</div><div className="under-actions"><Button variant="ghost" onClick={reset}><RotateCcw size={15}/> reset <kbd>:e</kbd></Button>{onRandom&&<Button variant="ghost" onClick={onRandom}><Shuffle size={15}/> random <kbd>:n</kbd></Button>}</div></div>}
    <div className="goal-row"><Timer size={14}/><span>PAR</span><b>{scenario.reference?.parKeystrokes??'—'} keys</b><span className="line"/><span>{scenario.validation.ignoreBlankLines!==false?'blank-only lines ignored':scenario.validation.ignoreTrailingWhitespace?'trailing whitespace ignored':'exact text match'}</span></div>
  </section>
}
function ping(){
  try{
    const context=new AudioContext();const start=context.currentTime+.01
    const tone=(frequency:number,delay:number,duration:number,volume:number,type:OscillatorType='sine')=>{const oscillator=context.createOscillator();const gain=context.createGain();const begins=start+delay;oscillator.type=type;oscillator.frequency.setValueAtTime(frequency,begins);gain.gain.setValueAtTime(.0001,begins);gain.gain.exponentialRampToValueAtTime(volume,begins+.004);gain.gain.exponentialRampToValueAtTime(.0001,begins+duration);oscillator.connect(gain).connect(context.destination);oscillator.start(begins);oscillator.stop(begins+duration);return oscillator}
    tone(659.25,0,.13,.055,'triangle')
    const bell=tone(1318.51,.085,.34,.075)
    tone(2637.02,.085,.2,.018)
    bell.onended=()=>{void context.close()}
  }catch{/* no audio */}
}
