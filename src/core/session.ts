...
export class GameSession {
  ...
  constructor(
    ...
    private readonly clock: Clock = performance,
  ) {}
  ...
  elapsed(now = this.clock.now()) {
    return this.status === 'ready'
      ? 0
      : Math.max(0, (this.status === 'completed' ? this.endTime : now) - this.startTime)
  }
}

export function betterRun(
  a: { elapsedMs: number; metrics: { keystrokes: number } },
  b?: { elapsedMs: number; metrics: { keystrokes: number } },
) {
  return (
    !b ||
    a.elapsedMs < b.elapsedMs ||
    (a.elapsedMs === b.elapsedMs && a.metrics.keystrokes < b.metrics.keystrokes)
  )
}
