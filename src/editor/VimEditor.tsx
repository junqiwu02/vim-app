...
export function VimEditor({
  ...
  useEffect(() => {
        ...
        EditorView.updateListener.of((update) => {
          if (update.docChanged) {
            callback.current.onChange(update.state.doc.toString(), {
              paste: update.transactions.some((t) => t.isUserEvent('input.paste')),
              undo: update.transactions.some((t) => t.isUserEvent('undo')),
            })
          }
        }),
      ],
    ...
    try {
      ...
      Vim.defineEx?.('n', 'n', () => callback.current.onNew?.())
      const status = document.querySelector('.cm-vim-panel')
      if (status) callback.current.onMode?.(status.textContent || 'NORMAL')
    } catch {
  ...
  return (
    <div
      className={`editor-shell ${focused ? 'is-focused' : 'is-unfocused'}`}
    ...
    >
      ...
      {!focused && (
        <button
          type="button"
          className="editor-focus-overlay"
          onMouseDown={(event) => {
            event.preventDefault()
            focusEditor()
          }}
