...
type State = {
  ...
  importScenarios: (items: Scenario[]) => Promise<void>
  updatePrefs: (p: Preferences) => void
  reload: () => Promise<void>
}
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const reload = async () => {
    const [s, r] = await Promise.all([persistence.getScenarios(), persistence.getRuns()])
    setImported(s)
    setRuns(r)
  }
  useEffect(() => {
    void Promise.all([persistence.getScenarios(), persistence.getRuns()]).then(([s, r]) => {
      setImported(s)
      setRuns(r)
    })
  }, [])
  ...
  const value: State = {
    ...
    async importScenarios(items) {
      await persistence.saveScenarios(items)
      await reload()
    },
    updatePrefs(p) {
      setPrefs(p)
...
export const useApp = () => {
  ...
  if (!c) throw new Error('Missing AppProvider')
  return c
}
